...
fn main() {
    ...
    println!("There is {} numbers with odd sums", odds);
    let common = mostcommonsum(&wordlist);
    println!("The biggest common sum is {} and it contains {} words", common.0, common.1);
    let pairswithdiff = findpairwithdiff(&wordlist, 11);
    println!("Found a pair with same sum and diff off {}: {}:{}", pairswithdiff.2, pairswithdiff.0, pairswithdiff.1);
}
